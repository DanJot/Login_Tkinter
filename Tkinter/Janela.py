...
def entrar(): #liga à BD, obtem credenciais
    ...
    username = luser.get()
    password = lpass.get()
    sql = f"SELECT * FROM login WHERE nome = '{username}'"
    cursor.execute(sql)
    resp = cursor.fetchone()
    if resp and resp[2] == password:  # resp[2] é a coluna da senha, se o utilizador existir, verificar a password com case sensitivity
...
def importar():
    ...
    b_close = customtkinter.CTkButton(jan3, text="Fechar", fg_color="indian red", hover_color="brown4", command=lambda: janela.destroy())
